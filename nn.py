...
class NeuralNetwork:
	...
	def crossEntropyLoss(self, Y, predictions):
		...
		predictions = np.where(predictions == 0, 1e-14, predictions)
	...
	def crossEntropyDelta(self, Y, predictions):
		...
		loss_delta = np.nan_to_num(-Y/(predictions+1e-16))
		return loss_delta
		###############################################
...
class FullyConnectedLayer:
	def __init__(self, in_nodes, out_nodes, activation):
		...
		self.weights = np.random.rand(in_nodes, out_nodes) * np.sqrt(2/in_nodes)
	...
	def softmax_of_X(self, X):
		...
		# softmax =  s / np.sum(s)
		# return softmax 
		mat = np.max(X, axis=1)[:, np.newaxis] 
		return np.exp(X - mat) / np.sum(np.exp(X - mat), axis=1)[:, np.newaxis]
		###############################################
	...
	def gradient_softmax_of_X(self, X, delta):
		...
		for (i,temp) in enumerate(X): 
			temp_j = -((np.array([temp]).T) @ (np.array([temp]))) 
			temp_j[np.diag_indices_from(temp_j)] += np.array([temp]).reshape(temp_j.shape[0],) 
			grad.append(delta[i] @ temp_j) 
	...
	def forwardpass(self, X):
		...
		if self.activation == 'relu':
			...
			self.data = self.relu_of_X(Y)

		elif self.activation == 'softmax':
			Y = np.dot(X, self.weights) + self.biases
			self.data = self.softmax_of_X(Y)
		else:
